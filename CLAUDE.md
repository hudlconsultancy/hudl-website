...
- **Slate** (`slides-builder`) — builds board packs and pitch decks as real PowerPoint files using python-pptx (a `build_*_pptx.py` script + the generated `.pptx`, saved to `clients/<slug>/reports/`). The `.pptx` opens directly in Google Slides (upload to Drive → Open with Google Slides) or PowerPoint — no script-running required.
